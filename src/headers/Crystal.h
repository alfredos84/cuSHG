...
//* Crystal grid
__constant__ const real_t LX	= 1000.0;		// Crystal width [μm]
...
//* Crystal properties
__constant__ const real_t deff			= 10.00E-6;	// Eff. second-order susceptibility [um/V]
__constant__ const real_t Lambda		= 7.97;			// grating period for QPM  [μm] 
__constant__ const real_t alpha_crp	= 0.*0.17e-6;	// pump linear absorption [1/μm]
__constant__ const real_t alpha_crs	= 0.*1.57e-6;		// signal linear absorption [1/μm]
__constant__ const real_t beta_crs	= 0.*5e-5;		// signal 2-photons absorption [μm/W]
...
{
	// Sellmeier equation for the refractive index 
	real_t A =  4.502483;
	real_t B =  0.007294;
	real_t C =  0.185087;
	real_t D =  -0.02357;
...
{	
	...
	std::cout << "Walk-off angle, ρ = " << rho << " rad" << std::endl;		 
